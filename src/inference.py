...
def run_docker_container(model, data_path, gpus, port, follow_logs=False):
    ...
    if num_shards > 1:
        ...
        docker_command = [
            ...
            '-p', f'{port}:80',
            '-v', f'{data_path}:/data',
            '-e', 'USE_PREFIX_CACHING=0',
            '-e', 'PREFIX_CACHING=0',
            '-e', 'PREFILL_CHUNKING=0',
            '-e', 'ATTENTION=flashdecoding',
            '--name', container_name,
            'ghcr.nju.edu.cn/huggingface/text-generation-inference:3.0.1',
            #'-e', 'USE_PREFIX_CACHING=0',
            #'-e', 'PREFIX_CACHING=0',
            #'-e', 'PREFILL_CHUNKING=0',
            #'-e', 'ATTENTION=paged'
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--data-path', type=str, required=False, help='Specify the path to the checkpoints, e.g., "/data"')
